"""
_scan_parameter_space.py — Comprehensive CPA2 flash parameter-space scan.

Runs stability test + multiple flash strategies across a dense (T, P, z_CO2) grid.
Saves all results to an npz file for later analysis and plotting.

Tracked quantities per grid point
──────────────────────────────────
  Stability test (6 initial guesses)
    - TPD value, converged flag, iteration count — per trial
    - Overall: stable/unstable, best TPD, which trial found lowest TPD
    - Number of trials that detected instability (TPD < -1e-7)

  Flash — 4 strategies compared
    1. Standard SSI + Wilson K          (baseline)
    2. Accelerated SSI + Wilson K       (acceleration benefit)
    3. Accelerated SSI + Stability K    (initial-guess benefit)
    4. Robust wrapper (stability→flash, fallbacks)  (production)
    For each: converged, iterations, residual norm, final m (acceleration factor)

  Phase & thermodynamic properties (from robust result)
    - phase label, beta, x_CO2, y_CO2, K_CO2, K_H2O
    - Z_aq, Z_vap, rho_aq, rho_vap
    - chi (association fractions), association ratios
    - swc_fallback_needed flag

  Timing
    - wall-clock time per point (includes all strategies)
"""
import time
import sys
import numpy as np
...
# Grid definition
# ═══════════════════════════════════════════════════════════════════════════════
T_grid = np.arange(273, 574, 5, dtype=float)       # 273–573 K, step 5 K  (61 pts)
P_grid = np.array([1, 2, 5, 10, 20, 50, 75, 100,
                   150, 200, 300, 400, 500, 600,
                   800, 1000], dtype=float)          # 16 pts, semi-log
...
N_TRIALS = 6   # number of stability initial guesses

print(f"Grid: {nT} T × {nP} P × {nz} z = {nT*nP*nz:,} points")
print(f"T: {T_grid[0]}–{T_grid[-1]} K   P: {P_grid[0]}–{P_grid[-1]} bar   "
      f"z_CO2: {z_grid[0]}–{z_grid[-1]}")

# ═══════════════════════════════════════════════════════════════════════════════
# Pre-allocate output arrays  (NaN = not computed / not applicable)
# ═══════════════════════════════════════════════════════════════════════════════
shape = (nT, nP, nz)
shape_trial = (nT, nP, nz, N_TRIALS)

# --- Stability test ---
stab_stable      = np.full(shape, False)                          # bool
stab_tpd_min     = np.full(shape, np.nan)                         # best TPD
stab_best_trial  = np.full(shape, -1, dtype=np.int8)              # index of best trial
stab_n_unstable  = np.zeros(shape, dtype=np.int8)                 # how many trials found TPD<-1e-7
stab_trial_tpd   = np.full(shape_trial, np.nan)                   # TPD per trial
stab_trial_conv  = np.full(shape_trial, False)                    # converged per trial
stab_trial_iter  = np.zeros(shape_trial, dtype=np.int16)          # iterations per trial
stab_K           = np.full((*shape, 2), np.nan)                   # K from best trial

# --- Flash: 4 strategies ---
# Strategy labels: 0=std_wilson, 1=acc_wilson, 2=acc_stab, 3=robust
STRAT_NAMES = ["std_wilson", "acc_wilson", "acc_stabK", "robust"]
N_STRAT = len(STRAT_NAMES)

flash_conv       = np.full((*shape, N_STRAT), False)              # converged
flash_iter       = np.zeros((*shape, N_STRAT), dtype=np.int16)    # iterations
flash_resid      = np.full((*shape, N_STRAT), np.nan)             # residual norm
flash_m          = np.full((*shape, N_STRAT), np.nan)             # final acceleration factor

# --- Phase & properties (from robust result) ---
phase_id         = np.zeros(shape, dtype=np.int8)
# Encoding: 0=failed, 1=single_phase, 2=single_liquid, 3=single_vapor, 4=two_phase
...
K_final          = np.full((*shape, 2), np.nan)   # [K_CO2, K_H2O]
Z_phase          = np.full((*shape, 2), np.nan)   # [Z_aq, Z_vap]
rho_phase        = np.full((*shape, 2), np.nan)   # [rho_aq, rho_vap] kg/L
chi_aq           = np.full((*shape, 2), np.nan)   # [chi_H2O, chi_CO2] aqueous
chi_vap          = np.full((*shape, 2), np.nan)   # [chi_H2O, chi_CO2] vapour
assoc_t          = np.full((*shape, 2), np.nan)   # association ratio [aq, vap]

# --- Diagnostics ---
swc_fallback     = np.full(shape, False)                          # needed swc→0 retry
robust_attempt   = np.zeros(shape, dtype=np.int8)
# Encoding: 0=stability(single), 1=acc+stabK, 2=acc+wilson,
#           3=swc0+stabK, 4=swc0+wilson, 5=failed
kij_arr          = np.full(shape[:1], np.nan)                     # kij per T
swc_arr          = np.full(shape[:1], np.nan)                     # swc per T
wall_time        = np.full(shape, np.nan)                         # seconds per point
...
stab_trial_labels = None   # filled on first call
...
for iT, T in enumerate(T_grid):
    ...
    for iP, P in enumerate(P_grid):
        for iz, zc in enumerate(z_grid):
            ...
            # ── Stability test ──────────────────────────────────────────
            stab = CPA2.stability_test(T, P, z, accelerated=True,
            ...
            tie0 = CPA2.tie_line_two_comp(T=T, P_bar=P, accelerated=False,
                                          **kw_base)
            ...
            tie1 = CPA2.tie_line_two_comp(T=T, P_bar=P, accelerated=True,
                                          **kw_base)
            ...
            if (K_s is not None and np.all(np.isfinite(K_s))
                    and np.all(K_s > 0)):
                tie2 = CPA2.tie_line_two_comp(
                    T=T, P_bar=P, K_init=K_s, accelerated=True, **kw_base)
            else:
                tie2 = tie1   # no stability K available, same as strategy 1
            ...
            # ── Strategy 3: "robust" — assemble from stability + best flash ──
            # Mirrors flash_co2_h2o_tpz_robust logic WITHOUT re-running
            ...
            if stab["stable"]:
                ...
                # Unstable → pick best converged flash result
                # Priority: acc+stabK > acc+wilson > swc0 fallback
                ...
                if has_stab_K and tie2["converged"]:
                    ...
                    # Try swc=0 fallback (only extra computation in scan)
                    kw_s0 = dict(kw_base, swc=0.0)
                    if has_stab_K:
                        tie_s0 = CPA2.tie_line_two_comp(
                            T=T, P_bar=P, K_init=K_s,
                            accelerated=True, **kw_s0)
                        if tie_s0["converged"]:
                            best_tie = tie_s0; attempt_code = 3
                    if best_tie is None:
                        tie_s0w = CPA2.tie_line_two_comp(
...
for s, name in enumerate(STRAT_NAMES):
    ...
    avg_it = np.nanmean(flash_iter[..., s][mask_2ph & flash_conv[..., s]])
    print(f"  {name:15s}: {nc:>6d}/{n_twoph}  "
          f"({100*nc/n_twoph:.1f}%)  avg iter = {avg_it:.1f}")

# Stability: how many initial guesses were needed?
print(f"\nStability initial guesses needed to detect instability "
      f"(unstable points, n={int(np.sum(~stab_stable))}):")
for ng in range(1, N_TRIALS + 1):
    n_ng = int(np.sum(stab_n_unstable[~stab_stable] >= ng))
    # "first ng trials sufficient" = best_trial index < ng
    n_first = int(np.sum(stab_best_trial[~stab_stable] < ng))
    print(f"  ≤{ng} guesses sufficient: {n_first} "
          f"({100*n_first/max(int(np.sum(~stab_stable)),1):.1f}%)")
...
# Speedup statistics
mask_both = flash_conv[..., 0] & flash_conv[..., 1]  # std & acc both converged
if np.any(mask_both):
    it_std = flash_iter[..., 0][mask_both].astype(float)
    it_acc = flash_iter[..., 1][mask_both].astype(float)
    ratio = it_std / np.maximum(it_acc, 1)
    print(f"\nAcceleration speedup (std_wilson vs acc_wilson, n={int(np.sum(mask_both))}):")
    ...
    pct_faster = 100 * np.mean(it_acc < it_std)
    print(f"  Accelerated faster: {pct_faster:.1f}%")

# ═══════════════════════════════════════════════════════════════════════════════
# Save to npz
# ═══════════════════════════════════════════════════════════════════════════════
outfile = "scan_results.npz"
np.savez_compressed(
    outfile,
    # Grid
