...
def ELV(x0, T, P, ms, params=None):
    ...
    try:
        ...
        # dF/d* = chi1w**2 * dD/d*, dG/d* = chi4w**2 * dE/d*  (see ecpa/elv.py)
...
def _cpa2_phase_check(T, P_bar, z_co2, params):
    """
    Quick CPA salt-free flash to classify likely phase state.
    Returns: 'two_phase', 'single_phase_liquid', 'single_phase_gas', or 'unknown'

    Classification for single-phase results uses the CPA phase compositions:
    ...
    CPA is salt-free, so these are conservative outer bounds — with salt, the
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            out = CPA.flash_co2_h2o_tpz(
                T=float(T), P_bar=float(P_bar), z_co2=float(z_co2))

        if out['phase'] == 'two_phase':
            return 'two_phase'

        # Single-phase: CPA convention is index 0=CO2, index 1=H2O
        ...
        # compositions look two-phase-like but CPA said single-phase —
...
def flash_co2_h2o_salt_1d(
    ...
    # ── optional CPA stability pre-check (disable for bulk scans) ────────
    if stability_check:
        _phase_hint = _cpa2_phase_check(T, P_bar, z_co2, params)
        if _phase_hint in ('single_phase_liquid', 'single_phase_gas'):
            raise RuntimeError(
                f"CPA salt-free pre-check indicates {_phase_hint} at "
...
def flash_co2_h2o_salt_ssi(
    ...
    if m_tot < 0.0:
        raise ValueError("m_tot must be >= 0.")
    if m_tot == 0.0:
        raise NotImplementedError("m_tot=0 (salt-free) not implemented; use CPA flash.")
