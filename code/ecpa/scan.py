...
def _cpa2_label(z_co2: float, T: float, P: float, params,
                fallback: str) -> str:
    """
    Run a fast CPA salt-free phase check and return a specific single-phase
    ...
    return fallback   # CPA uncertain → keep the algorithmic failure label
...
def _scan_worker_task(task):
    ...
    try:
        ...
        if "sign change" in msg and _W_flash_algo == "brent":
            ...
            # Both Brent and SSI failed.  Use CPA (salt-free, fast) to decide:
            # if CPA says single-phase, salt only tightens the window, so eCPA
...
def run_flash_scan(
    ...
    use_cpa2_precheck   : if True, skip points identified as single-phase by CPA
...
def print_scan_summary(df_results: pd.DataFrame) -> None:
    ...
    interp = {
        ...
        "out_of_range":       "outside table P range or CPA single-phase",
        "salting_out":        "salt displaces CO2 (x4w<0)",
        "single_phase_stable":"flash failed + ecpa_stability confirmed single-phase",
        "single_phase_gas":   "flash failed + CPA confirmed single-phase gas",
        "single_phase_liquid":"flash failed + CPA confirmed single-phase liquid",
        "no_sign_change":     "Brent + SSI + CPA all uncertain — genuine flash failure",
        "elv_solver":         "ELV Newton failed",
        "cache_empty":        "continuation cache empty at ms=0",
        "ssi_no_converge":    "SSI did not converge — CPA also uncertain",
