...
1. Extend CPA_ELV_all.parquet to cover T = 273–282 K and T = 538–700 K
   ...
   Columns computed by CPA.py: Z_W, xw_W, Z_C, xw_C, chiw_W, chiw_C.
...
for iT_miss, T in enumerate(missing_T):
    ...
    n_conv = 0
    for P in P_ELV:
        tie = CPA.tie_line_two_comp(T=T, P_bar=P, kij12=kij, swc=swc, **kw_elv)
...
for iT, T in enumerate(T_grid):
    kij = CPA.kij_ecpa(float(T))
    swc = CPA.s14_ecpa(float(T))
