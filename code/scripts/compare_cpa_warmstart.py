...
Method A — CPA (no table):
    CPA.flash_co2_h2o_tpz_robust — 6-trial Michelsen stability +
...
if __name__ == '__main__':
    ...
    print(f"{'-'*60}")
    print(f"{'A: CPA robust (no table)':<35}  "
    ...
    ax.hist(merged['cpa_t_ms'], bins=bins_t, density=True,
            color='steelblue', alpha=0.65, label='Robust eCPA solver')
    ax.hist(merged['kv_t_ms'],  bins=bins_t, density=True,
            color='tomato',    alpha=0.65, label='Table warm-start SSI')
    ...
    ax.set_xlabel('Robust eCPA solver', fontsize=9)
    ax.set_ylabel('Table warm-start SSI', fontsize=9)
