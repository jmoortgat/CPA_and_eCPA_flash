...
def make_comparison_figure(result_a: dict, result_b: dict,
    ...
    for col, (fd, res) in enumerate(zip([fa, fb], [result_a, result_b])):
        for row, (key, row_label, cmap, shared_vlim, vlim_fixed) in enumerate(rows):
            ...
            if row == 0:
                ax.set_title(col_labels[col], fontsize=11, fontweight='bold', pad=4)
            if col == 0:
...
def make_trapping_figure(result_a: dict, result_b: dict,
    ...
    ax.set_xlabel('Simulation time [yr]', fontsize=11, fontweight='bold')
    ax.set_ylabel('Fraction of in-situ CO₂  [%]', fontsize=11, fontweight='bold')
    ax.set_xlim(left=0)
    ax.set_ylim(0, 100)
    ax.legend(fontsize=9, loc='center right', prop={'weight': 'bold'})
