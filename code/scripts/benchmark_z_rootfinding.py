...
Benchmark: compressibility-factor (Z) root-finding cost across three EoS levels.
  ...
  Level 2 – CPA (Wertheim 4C association, salt-free)
      Z and (χ_H₂O, χ_CO₂) are coupled.  CPA.ZChi() uses a 12-point
...
def _srk_AB(T, P_bar, x_h2o, x_co2):
    ...
    # Re-use CPA parameter machinery (eCPA param set)
    ep = CPA._eos_aq(T, P_bar, np.array([x_co2, x_h2o]), kij12=CPA.kij_ecpa(T))
    return ep["A"], ep["B"]
...
def _z_cpa_one_phase(T, P_bar, x_h2o, x_co2, phase="aqueous"):
    ...
    if phase == "aqueous":
        ep = CPA._eos_aq(T, P_bar, n, kij12=kij)
    else:
        ep = CPA._eos_vap(T, P_bar, n, kij12=kij)
    ...
    counter = [0]
    _orig = CPA._funz_combined

    def _counted(Z, A_, B_, n_, Kapa_, Eps_, swc_):
        counter[0] += 1
        return _orig(Z, A_, B_, n_, Kapa_, Eps_, swc_)

    with patch.object(CPA, "_funz_combined", _counted):
        Z, Chi, Chi1 = CPA.ZChi(A, B, n, Kapa, Eps, swc)
...
def _ecpa_cold_starts(T, P_bar, x1w, x4w):
    ...
    ep   = CPA._eos_aq(T, P_bar, n, kij12=kij)
    try:
        Z_cpa, chi_h2o, _, = CPA.ZChi(ep["A"], ep["B"], n, ep["Kapa"], ep["Eps"], swc)
