...
(505{,}400 grid points, no explicit CO$_2$ feed fraction $z$ dimension since K-values are
essentially $z$-independent), enabling warm-started flash calculations that
...
The table deliberately omits the overall CO$_2$ mole fraction $z$ as an independent
dimension.
This is justified thermodynamically: the K-values (and hence the tie-line endpoints
$x_{\text{CO}_2}$ and $y_{\text{H}_2\text{O}}$) depend on $(T, P, m_s^\mathrm{aq})$ but
not on $z$.
Numerically, we verified that at typical subsurface conditions ($T = 323$\,K,
$P = 100$\,bar, $m_s = 1$\,mol\,kg$^{-1}$) the spread of $K$-values across the
full two-phase $z$ range is less than 0.15\%, confirming that a $z$-free table
introduces negligible warm-start error.

Each cell is solved using the full hierarchical \texttt{ecpa\_stability\_flash}
...
At $T = 252\,^\circ$C, up to $\approx 30$\,mol\% of the CO$_2$-rich phase is water,
...
Table~\ref{tab:benchmark} compares the wall time and SSI iteration count of three flash
strategies, evaluated at $T = 125\,^\circ$C, $z_{\text{CO}_2} = 0.5$, $m_s = 1.0$\,mol\,kg$^{-1}$,
over 30 pressure points spanning 10--1500\,bar.
...
\begin{table}[htb]
  ...
  \caption{Benchmark of flash strategies at $T = 125\,^\circ$C, $z_{\text{CO}_2} = 0.5$,
           $m_s = 1.0$\,mol\,kg$^{-1}$, 30 pressure points (10--1500\,bar).}
  \label{tab:benchmark}
  \begin{tabular}{lrrrr}
    \toprule
    Strategy & Mean iters & Total time & Time/call & Speedup \\
    \midrule
    Cold SSI (Wilson init)          & 7.4  & 0.18\,s & 6\,ms  & 1.0$\times$ (baseline) \\
    Fast: 3D table warm, no stab.   & 4.8  & 0.09\,s & 3\,ms  & \textbf{2.1}$\times$ \\
    Fast + forced stability check   & ---  & 0.39\,s & 13\,ms & 0.5$\times$ \\
    \bottomrule
  \end{tabular}
\end{table}

The fast flash with selective stability checking achieves a 2.1$\times$ speedup over cold-start
SSI, reducing the mean iteration count from 7.4 to 4.8.
Forcing the stability test on every call negates this advantage entirely: the Michelsen TPD
evaluation ($\sim$10\,ms per call for six guesses) costs more than three times the warm-started
flash alone ($\sim$3\,ms per call).
...
K-value SSI at $m_s \to 0$ (querying the $m_s = 0$ slice of the 3D table).
...
relevant to geological CO$_2$ storage, we computed the three-dimensional solution table
described in Section~\ref{sec:table}, covering the grid summarised in
...
essentially $z$-independent, as discussed in Section~\ref{sec:table}).
...
Every grid point is classified---100.000\% convergence is achieved over the
...
\begin{table}[htb]
  ...
  \begin{tabular}{lrl}
    ...
    \midrule
    Two-phase, converged          & 289{,}062 & Valid two-phase split; SSI converged \\
    Single-phase (stable)         & 216{,}337 & Michelsen stability test: no phase split \\
    Failed (non-converged)        &       1   & Degenerate low-$P$ condition ($T=280$\,K, $P=1.7$\,bar) \\
    \midrule
    Total classified              & 505{,}400 & 100.000\% \\
    \bottomrule
  \end{tabular}
\end{table}

The single failure occurs at a degenerate near-atmospheric pressure ($P = 1.7$\,bar,
$T = 280$\,K) well outside practical CO$_2$ storage conditions; it has no effect on
warm-start quality for the target application.

\paragraph{Two-phase window.}
Figure~\ref{fig:ecpa_phase_map} shows the two-phase region in the $(T, P)$ plane
at a representative feed $z = 0.5$ for six NaCl molalities.
The gold region is two-phase; the blue region is single-phase.
Three features stand out.
First, the two-phase window is wide: it spans the full pressure range from 1 to
2000\,bar at most temperatures below $\sim$250\,$^\circ$C.
Second, the window narrows as NaCl molality increases
(left to right), directly reflecting the salting-out effect on CO$_2$ solubility.
Third, at the highest temperatures the two-phase region shrinks rapidly as CO$_2$
and water approach full miscibility.

\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{../figures/scan_v4/ecpa_phase_map.pdf}
  \caption{Two-phase (gold) and single-phase (blue) regions in the $(T, P)$ plane
           at a representative overall CO$_2$ feed $z = 0.5$,
           for six NaCl molalities (columns, $m_s = 0$--$6$\,mol\,kg$^{-1}$).
           The broad gold region confirms that CO$_2$--brine systems are two-phase
           over most of the geologically relevant $T$--$P$ range.
           Increasing NaCl molality progressively narrows the two-phase window
           (salting-out), while high temperatures ($T \gtrsim 300\,^\circ$C) drive
           the system toward complete miscibility.}
...
at a representative feed $z = 0.5$; red curves trace the dew- and bubble-point boundaries.
...
logarithmic colour scale spanning 0.05--25\,mol\%, at a representative feed $z = 0.5$.
...
Increasing NaCl molality (left to right) progressively contracts the
...
\begin{figure}[htb]
  ...
  \caption{Equilibrium CO$_2$ mol\% in the aqueous phase ($x_{\text{CO}_2}$)
           ...
           $m_s$ quantifies the salting-out effect over the full $T$--$P$ domain.}
...
Figure~\ref{fig:ecpa_ssi_heatmap} maps the mean wall time per flash call
across the $T$--$P$ plane for the same six NaCl molalities,
restricted to two-phase conditions.
Wall time serves as a proxy for solver effort: it reflects both the number of
SSI iterations and the cost per iteration.
Times are lowest ($\lesssim 5$\,ms) in the interior of the two-phase region
at moderate conditions, where the warm-started SSI converges in 2--3 steps.
Times rise near the phase boundaries---particularly the upper pressure boundary
(bubble point) and near the CO$_2$ critical temperature---where K-values are close
to unity and more SSI iterations are required.
...
\begin{figure}[htb]
  ...
  \caption{Mean wall time per flash call (ms) for two-phase conditions,
           over the $T$--$P$ plane for six NaCl molalities (columns),
           from the 3D scan (505{,}400 grid points).
           Grey regions are single-phase at those $(T,P,m_s)$ conditions.
           The colormap is log-scaled; wall time serves as a proxy for solver
           effort.
           Times are lowest in the interior of the two-phase region
...
\begin{table}[htb]
  ...
  \begin{tabular}{llrrrrr}
    ...
    CO$_2$+NaCl    & eCPA (warm SSI)    & 689      & 8.0  &  8 & 14  & $\sim$26\,ms  \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item A precomputed three-dimensional solution table (361T $\times$ 100P $\times$
