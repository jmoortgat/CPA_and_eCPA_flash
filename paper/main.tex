...
To demonstrate the applicability of the flash algorithms developed in this work within
a reservoir simulation context, we implemented an isothermal IMPES
(implicit-pressure, explicit-composition) compositional simulator for CO$_2$ injection
into a 2D aquifer.
The primary goal is to stress-test the robustness and efficiency of the stability and
flash routines under the repetitive, varying-composition calls of a time-stepping loop,
not to produce physically predictive reservoir simulations.
...
mixed finite element (MFE, RT$_0$) scheme \citep{Aavatsmark2003}.
...
advection of the overall CO$_2$ mole fraction $z$.
The transport sub-step size is constrained by a CFL condition based on the maximum
slope of the molar fractional flow $\partial F_z / \partial z$, estimated by a
one-dimensional scan at startup.
Production wells are controlled by a fixed bottom-hole pressure (BHP) via standard
Peaceman well indices; the injector delivers a fixed volumetric rate.
...
Two-phase cells with $z < z_\mathrm{lo}$ or $z > z_\mathrm{hi}$ are treated as trivially
single-phase and bypass the flash entirely.
...
and efficiently at every cell of a $50{\times}50$ grid over 300 time steps spanning 5 years.
...
Full simulation parameters are listed in Table~S\ref{tab:sim_setup} in the Supplemental
Information.
...
Given that CO$_2$ solubility in water is already low---on the order of 1--2\,mol\,kg$^{-1}$
under typical storage conditions---the further reduction from salting-out has negligible
impact on the high-rate displacement dynamics on these time scales.
...
and only $m_c \approx 0.63$\,mol\,kg$^{-1}$ at $m_s = 4$\,mol\,kg$^{-1}$---a reduction
of approximately a factor of two, consistent with the salting-out effect seen in the
validation data (Section~\ref{sec:co2nacl}).
The white region around the central injector marks cells where the brine has been fully
expelled into the CO$_2$-rich phase (complete drying out, $S_g \to 1$), leaving no
aqueous phase to dissolve CO$_2$ into.
Correctly capturing this drying-out regime requires a robust stability test: a
simplified flash that merely solves the two-phase equilibrium without first checking
single-phase stability would fail or produce unphysical compositions for these cells.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{../figures/demo/comparison_final.png}
...
cases.
Within the first year, structural trapping dominates as the injected CO$_2$ rapidly
fills pore space.
Over the remaining four years, solubility trapping grows steadily as CO$_2$ dissolves
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.80\textwidth]{../figures/demo/trapping_comparison.png}
...
grid cells.
Cells identified as trivially single-phase (overall CO$_2$ fraction below the two-phase
entry threshold $z_\mathrm{lo} = 0.015$ or above $z_\mathrm{hi} = 0.97$) bypass the flash
entirely; the remaining two-phase candidates are dispatched in parallel across all
available CPU cores.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrr}
    ...
    Time steps                          & 300   & 300   \\
    Mean two-phase cells / step         & ---   & ---   \\
    Mean flash wall time / step (s)     & ---   & ---   \\
    Flash fraction of step cost         & ---   & ---   \\
    Flash throughput (calls\,s$^{-1}$)  & ---   & ---   \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{llr}
    ...
    Temperature (isothermal)           & $T$              & 350\,K ($77\,^\circ$C) \\
