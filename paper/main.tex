...
nine false-positive two-phase predictions at $T = 463$--$493$\,K, $P = 10$\,bar
...
\paragraph{Benchmark.}
On a cold-start grid ($T = 313$--$493$\,K, $P = 10$--$800$\,bar, six CO$_2$ feed
...
\begin{itemize}
  \item $T$: 31 points from 273 to 728\,K (approximately uniform 15-K spacing),
...
\begin{table}[htb]
  ...
  \caption{Comparison of six flash strategies across 9{,}825 two-phase conditions
           ...
           extended to $T = 273$--$673$\,K (99.9\% of two-phase points).
...
The table was extended from the original $T = 283$--$533$\,K range to cover
$T = 273$--$673$\,K by running the flash solver at the 35 scan-grid temperatures
previously outside the table; above $673$\,K no two-phase region exists at
$P \leq 1500$\,bar.
...
In the near-CO$_2$-critical region ($T \approx 280$--$330$\,K) the Newton step
...
\begin{table}[htb]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    Temperature $T$ & 288--633\,K & 70 (5\,K steps) \\
...
Figure~\ref{fig:ecpa_phase_map} shows how the two-phase window evolves across the
$T$--$P$ plane for representative NaCl molalities and CO$_2$ feed fractions.
Figure~\ref{fig:ecpa_composition_maps} presents the equilibrium aqueous CO$_2$ content
($x_{\mathrm{CO_2}}^{\mathrm{aq}}$, mol\%) and H$_2$O content in the CO$_2$-rich
phase ($x_{\mathrm{H_2O}}^{\mathrm{CO_2}}$, mol\%) as colour maps over the $T$--$P$
plane for selected conditions.
Figure~\ref{fig:ecpa_ssi_heatmap} extends the iteration-count analysis of
Fig.~\ref{fig:ssi_heatmap} to the ternary system, showing the number of SSI iterations
required across the $T$--$P$ plane for several NaCl molalities.
...
\begin{figure}[htb]
  ...
  \caption{SSI + Newton polish iteration breakdown and wall-time ratio across the
           ...
           ($T \approx 280$--$330$\,K) requires the most SSI iterations even with
           table warm-start.
