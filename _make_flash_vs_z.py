...
fig.suptitle(
    r"$P = 200\,\mathrm{bar}$,"
    r"  $m_\mathrm{feed} = 1.0\,\mathrm{mol\,kg^{-1}}$ NaCl",
    fontsize=12,
)
...
for ax, key, ylabel, title, ymin, ymax, _, __ in panels:
    ...
    ax.set_xlabel(z_label, fontsize=9)
...
ax_ms.axhline(1.0, color="k", lw=0.8, ls="--", zorder=3)
ax_ms.text(0.02, 1.02, r"$m_s^\mathrm{aq} = m_\mathrm{feed}$",
           transform=ax_ms.get_yaxis_transform(), fontsize=7.5, va='bottom')
