...
def _ZChi_newton_counted(A, B, n, Kapa, Eps, swc, Z0, Chi0, Chi10):
    """
    Identical to CPA2._ZChi_newton but returns (Z, Chi, Chi1, n_newton_iters).
    ...
    for nit in range(CPA2._ZCHI_NEWTON_MAXITER):
        if Z <= B:
            raise RuntimeError("Z <= B")
        eta   = B / (4.0 * Z)
        g     = CPA2._g(eta);  dg = CPA2._dgdeta(eta);  d2g = CPA2._d2gdeta2(eta)
        ...
        if _no_cross:
            ...
            if max_resid < CPA2._ZCHI_NEWTON_TOL:
                break
...
for k, (iT, iP, iz) in enumerate(sample):
    ...
    chi1_W = CPA2._chi1_from_Z_chi(Z_W, chi_W, ep_aq["B"], x[1],
                                    ep_aq["Kapa"], ep_aq["Eps"], swc)
    chi1_C = CPA2._chi1_from_Z_chi(Z_C, chi_C, ep_vap["B"], y[1],
    ...
    try:
        ...
        chi1_Wp = CPA2._chi1_from_Z_chi(Z_Wp, chi_Wp, ep_aq["B"], x[1],
                                         ep_aq["Kapa"], ep_aq["Eps"], swc)
        chi1_Cp = CPA2._chi1_from_Z_chi(Z_Cp, chi_Cp, ep_vap["B"], y[1],
                                         ep_vap["Kapa"], ep_vap["Eps"], swc)
    ...
    try:
        t_eos_cpa[k] = _time(lambda: (CPA2._eos_aq(T, P, x, kij),
                                       CPA2._eos_vap(T, P, y, kij)))

        t_zchi_cpa[k] = _time(lambda: (
            CPA2._ZChi_newton(ep_aq["A"], ep_aq["B"], x,
                              ep_aq["Kapa"], ep_aq["Eps"], swc, Z_W, chi_W, chi1_W),
            CPA2._ZChi_newton(ep_vap["A"], ep_vap["B"], y,
                              ep_vap["Kapa"], ep_vap["Eps"], swc, Z_C, chi_C, chi1_C)))

        t_lnphi_cpa[k] = _time(lambda: (
            CPA2._lnphi_aq (ep_aq,  Z_W, chi_W,  chi1_W),
            CPA2._lnphi_vap(ep_vap, Z_C, chi_C,  chi1_C)))
    except Exception:
        n_skip += 1
        continue
    ...
    t_lnphi_srk[k] = _time(lambda: (
        CPA2._lnphi_aq (ep_aq,  Z_W, 1.0, 1.0),
        CPA2._lnphi_vap(ep_vap, Z_C, 1.0, 1.0)))
