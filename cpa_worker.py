"""
cpa2_worker.py — top-level worker for parallel CPA2 (salt-free CO₂ + H₂O)
...
# CPA2.py lives in the same directory as this worker
_HERE = os.path.dirname(os.path.abspath(__file__))
if _HERE not in sys.path:
    sys.path.insert(0, _HERE)

import CPA2  # noqa: E402
...
def compute_one_point(task: tuple) -> dict:
    ...
    for P in sorted(P_scan):
        try:
            res = CPA2.flash_co2_h2o_tpz(T=float(T), P_bar=float(P),
                                          z_co2=float(z_co2))
