...
def _cpa2_vle_worker(args):
    """Top-level worker for plot_cpa2_vs_exp (parallel CPA2 flashes)."""
...
def plot_cpa2_vs_exp(T_list, exp_df, N_P=40, n_workers=48,
                     save_path=None):
    """CPA2 flash vs experimental VLE: grid of T rows × 2 columns."""
    tasks   = []
    P_grids = {}
    for T_K in T_list:
    ...
    fig.suptitle("CPA2 vs. Experiment — CO$_2$–H$_2$O VLE", fontsize=12, y=0.99)
    _save(fig, save_path)
    plt.show()
    return fig
...
def plot_combined_vs_exp(T_list, exp_df, CPA_GROUPS, CPA_TEMPS,
                          ...
                          P_MAX=1000.0,
                          save_path=None):
    """eCPA table + CPA2 flash vs experiment: grid of T rows × 2 columns."""
    ...
    fig.suptitle("eCPA Table & CPA2 Flash vs. Experiment — CO$_2$–H$_2$O VLE",
                 fontsize=12, y=0.99)
    _save(fig, save_path)
    plt.show()
    return fig
...
def plot_convergence_map(df_results, z_co2_values, ms_values,
    ...
    etype_code = {
        ...
        "out_of_range":       1,
        "salting_out":        2,
        "no_sign_change":     3,   # flash failed, CPA2 uncertain
        "single_phase_stable":3,   # flash failed, ecpa_stability confirmed
        "single_phase_gas":   3,   # flash failed, CPA2 confirmed gas
        "single_phase_liquid":3,   # flash failed, CPA2 confirmed liquid
        "ssi_no_converge":    3,
        "elv_solver":         4,
        "cache_empty":        4,
        "runtime_other":      4,
    ...
    marker_map   = {
        ...
        "single_phase_gas":   ("#17becf", "<",  18),  # teal ◄ – CPA2 confirmed gas
        "single_phase_liquid":("#17becf", ">",  18),  # teal ► – CPA2 confirmed liquid
    ...
    legend_handles += [
        ...
        Line2D([0],[0], marker="<", color="w", markerfacecolor="#17becf",
               markersize=8, label="single_phase_gas — CPA2 confirmed (◄)"),
        Line2D([0],[0], marker=">", color="w", markerfacecolor="#17becf",
               markersize=8, label="single_phase_liquid — CPA2 confirmed (►)"),
    ]
...
def plot_phase_envelope(envelopes_ecpa, z_co2_values, ms_values,
    ...
    eCPA solid/dashed; CPA2 salt-free dotted (if provided).
            ...
            if df_cpa2 is not None and len(df_cpa2) > 0:
                ...
                if len(df_c2_lo) > 0:
                    ax.plot(df_c2_lo["T"], df_c2_lo["P_lo"], color="black",
                            lw=1.5, ls=":", label="bubble pt (CPA2)")
    ...
    plt.suptitle("CO₂–H₂O–NaCl Phase Envelope (eCPA EoS)",
                 fontsize=14, y=1.01)
    plt.tight_layout()
    _save(fig, save_path)
    plt.show()
