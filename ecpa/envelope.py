"""
Phase-envelope computation: eCPA (from scan bisection) and CPA2 (parallel).
"""
...
def build_cpa2_envelope(
    ...
    """
    Build CPA2 salt-free phase envelopes in parallel using cpa2_worker.
    ...
    if os.path.exists(cache_file) and not force_recompute:
        ...
        print(f"Resuming CPA2 envelope from {cache_file}  ({n_cached} pts cached)")
    else:
        if force_recompute and os.path.exists(cache_file):
            os.remove(cache_file)
            print(f"force_recompute=True — deleted {cache_file}")
