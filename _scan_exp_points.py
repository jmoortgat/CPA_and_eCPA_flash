"""
_scan_exp_points.py — Run CPA2 stability + flash at every experimental CO2-H2O data point.

Loads CO2_WATER_exp.parquet (631 points, T=273–623 K, P=5–3500 bar),
runs the full CPA2 hierarchical stability+flash at each (T, P) with z_CO2=0.5,
...
for i in range(N_all):
    ...
    tie0 = CPA2.tie_line_two_comp(T=T, P_bar=P, accelerated=False, **kw_base)
    ...
    tie1 = CPA2.tie_line_two_comp(T=T, P_bar=P, accelerated=True, **kw_base)
    ...
    if K_s is not None and np.all(np.isfinite(K_s)) and np.all(K_s > 0):
        tie2 = CPA2.tie_line_two_comp(T=T, P_bar=P, K_init=K_s,
    ...
    if stab["stable"]:
        ...
        if has_stab_K and tie2["converged"]:
            ...
            if has_stab_K:
                tie_s0 = CPA2.tie_line_two_comp(T=T, P_bar=P, K_init=K_s,
                                                accelerated=True, **kw_s0)
                if tie_s0["converged"]:
                    best_tie = tie_s0; attempt_code = 3
            if best_tie is None:
                tie_s0w = CPA2.tie_line_two_comp(T=T, P_bar=P,
                                                 accelerated=True, **kw_s0)
