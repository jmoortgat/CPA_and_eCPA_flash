"""
Tight 2D heatmap grid: CO2 in aqueous phase (x4w, mol-%).
...
Output: figures/scan_v3/ecpa_composition_aq_grid.png/.pdf
"""
...
# pcolormesh needs cell edges; compute midpoint-based edges
def _edges(v):
    mid = 0.5 * (v[:-1] + v[1:])
    lo  = v[0]  - (mid[0]  - v[0])
    hi  = v[-1] + (v[-1]  - mid[-1])
...
def _find_envelope(is_2ph):
    ...
    'dew'    = lowest-P two-phase boundary (lower edge of two-phase window).
    'bubble' = highest-P two-phase boundary (upper edge of two-phase window).
    """
    nT, nP = is_2ph.shape
...
T_C      = T_grid - 273.15          # K → °C
T_edges  = _edges(T_C)
logP     = np.log10(P_grid)
logP_edges = _edges(logP)

T_TICKS    = [50, 150, 250, 350]    # °C, away from panel edges (~15–360 °C)
...
# ── figure ─────────────────────────────────────────────────────────────────────
FIG_W = 7.2      # inches (full text width)
FIG_H = 4.0
...
for ri, iz in enumerate(IZ_ROWS):
    for ci, ims in enumerate(IMS_COLS):
        ...
        # data slice: transpose to (nP, nT) for pcolormesh(T_C, logP, data)
        val  = x4w[:, :, iz, ims].T * 100          # (50, 70) mol-%
        mask = is_two[:, :, iz, ims].T              # (50, 70) bool

        # grey background for single-phase
        ...
        # T-axis: bottom row only; suppress last tick label on non-final columns
        if ri == NR - 1:
...
# ── row labels (left of each row, horizontal) ─────────────────────────────────
...
cb.set_label(r"$x_{\text{CO}_2}$ (mol-\%)", fontsize=8, labelpad=4)
cb_ticks = [0.05, 0.1, 0.5, 1, 2, 5, 10, 25]
cb.set_ticks(cb_ticks)
cb.set_ticklabels([f"{v:g}" for v in cb_ticks], fontsize=6.5)

# ── save ───────────────────────────────────────────────────────────────────────
outname = "ecpa_composition_aq_grid"
for ext in ("png", "pdf"):
